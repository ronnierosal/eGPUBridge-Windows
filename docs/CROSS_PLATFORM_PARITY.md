...
| EGB-C10 | `plugin.log`, recent events, and diagnostic status | JSONL logs with shared event names and Windows-native evidence | SteamOS: **Implemented**, with redacted recent events **In flight** at `54b0ef2`. Windows: operation-scoped transition logs **Implemented**; identifier redaction and device events **In flight** at `662fb2b`. | Evidence fields differ; event meaning should not. | Redact local user, host, network, and unique monitor identifiers while retaining bounded PCI evidence needed for validation. | S-DEV-CI covers recent-event redaction. W-CI asserts transition sequences; W-AWARE-CI covers redaction. |
